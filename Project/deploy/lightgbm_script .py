...
# test set. LGBMClassifier supports `class_weight` natively (same sklearn-style
# API), so it's passed straight to the model.
...
def train_model(X_train, y_train, plot_name, n_estimators, max_depth, learning_rate, class_weight=None):

    mlflow.set_experiment('loan-default-detection')
    with mlflow.start_run() as run:
        mlflow.set_tag('clf', 'lightgbm')
        ...
        clf = LGBMClassifier(n_estimators=n_estimators, max_depth=max_depth, learning_rate=learning_rate,
                              class_weight=class_weight, random_state=RANDOM_STATE, verbose=-1)
        ...
        mlflow.lightgbm.log_model(clf, name=f'{clf.__class__.__name__}_{plot_name}')
