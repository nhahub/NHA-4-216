...
# test set. DecisionTreeClassifier supports `class_weight` natively, so it's
...
def train_model(X_train, y_train, plot_name, max_depth, min_samples_leaf, class_weight=None):
...
def main(max_depth: int, min_samples_leaf: int):
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='with-class-weights',
                max_depth=max_depth, min_samples_leaf=min_samples_leaf, class_weight=class_weights)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_depth', '-d', type=int, default=12)
    parser.add_argument('--min_samples_leaf', '-m', type=int, default=15)
