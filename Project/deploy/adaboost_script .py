...
# test set. AdaBoostClassifier has no `class_weight` parameter, so the same
# weights are applied as per-sample weights at fit time.
...
def train_model(X_train, y_train, plot_name, n_estimators, learning_rate, sample_weight=None):

    mlflow.set_experiment('loan-default-detection')
    with mlflow.start_run() as run:
        mlflow.set_tag('clf', 'adaboost')
...
def main(n_estimators: int, learning_rate: float):
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='without-imbalance',
                n_estimators=n_estimators, learning_rate=learning_rate, sample_weight=None)
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='with-class-weights',
                n_estimators=n_estimators, learning_rate=learning_rate, sample_weight=sample_weight_balanced)
