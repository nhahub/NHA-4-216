...
# without inventing data. XGBoost doesn't take a class_weight dict directly,
# so its native equivalent, `scale_pos_weight`, is used instead.
...
def train_model(X_train, y_train, plot_name, n_estimators, max_depth, learning_rate, scale_pos_weight=1):
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.xgboost.log_model(clf, name=f'{clf.__class__.__name__}_{plot_name}')
...
def main(n_estimators: int, max_depth: int, learning_rate: float):
    ...
    ## 2. with considering the imbalance using scale_pos_weight
    train_model(X_train=X_train_final, y_train=y_train, plot_name='with-scale-pos-weight',
                n_estimators=n_estimators, max_depth=max_depth, learning_rate=learning_rate,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_depth', '-d', type=int, default=6)
    parser.add_argument('--learning_rate', '-l', type=float, default=0.1)
    args = parser.parse_args()

    ## Call the main function
    main(n_estimators=args.n_estimators, max_depth=args.max_depth, learning_rate=args.learning_rate)
