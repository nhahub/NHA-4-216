...
# test set. Logistic Regression supports `class_weight` natively, so it's
...
def train_model(X_train, y_train, plot_name, max_iter, class_weight=None):
...
def main(max_iter: int):
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='with-class-weights',
                max_iter=max_iter, class_weight=class_weights)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iter', '-i', type=int, default=1000)
    args = parser.parse_args()

    ## Call the main function
    main(max_iter=args.max_iter)
