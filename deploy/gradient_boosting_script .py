...
# without inventing data. GradientBoostingClassifier has no `class_weight`
...
def train_model(X_train, y_train, plot_name, n_estimators, max_depth, learning_rate, subsample, sample_weight=None):

    mlflow.set_experiment('loan-default-detection')
    with mlflow.start_run() as run:
        mlflow.set_tag('clf', 'gradient_boosting')
...
def main(n_estimators: int, max_depth: int, learning_rate: float, subsample: float):
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='without-imbalance',
                ...
                subsample=subsample, sample_weight=None)
    ...
    train_model(X_train=X_train_final, y_train=y_train, plot_name='with-class-weights',
                n_estimators=n_estimators, max_depth=max_depth, learning_rate=learning_rate,
                subsample=subsample, sample_weight=sample_weight_balanced)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subsample', '-s', type=float, default=0.8)
