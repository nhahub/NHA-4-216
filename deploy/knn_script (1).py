...
# KNeighborsClassifier has no `class_weight` parameter and its `.fit()` does
# not accept `sample_weight` either (it's a purely distance-based, lazy
# learner). None of the imbalance-handling techniques used elsewhere in this
# project (class_weight / sample_weight / SMOTE) apply to it, so — matching
# the notebook — KNN is trained as-is, with no imbalance handling.


## --------------------- Modeling ---------------------------- ##

def train_model(X_train, y_train, plot_name, n_neighbors):
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_params({'n_neighbors': n_neighbors})
...
def main(n_neighbors: int):

    # ---------------- Calling the above function -------------------- ##

    ## Only one run: no imbalance-handling technique applies to KNN
    train_model(X_train=X_train_final, y_train=y_train, plot_name='knn',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_neighbors', '-k', type=int, default=7)
